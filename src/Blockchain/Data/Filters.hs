...
module Blockchain.Data.Filters where
...
getBlkFilter :: (E.Esqueleto query expr backend) => (expr (Entity BlockDataRef), expr (Entity AddressStateRef), expr (Entity RawTransaction), expr (Entity Block))-> (T.Text, T.Text) -> expr (E.Value Bool)
...
getAccFilter :: (E.Esqueleto query expr backend) => (expr (Entity AddressStateRef))-> (T.Text, T.Text) -> expr (E.Value Bool)
...
getStorageFilter :: (E.Esqueleto query expr backend) => (expr (Entity Storage), expr (Entity AddressStateRef)) -> (T.Text, T.Text) -> expr (E.Value Bool)
...
getLogFilter :: (E.Esqueleto query expr backend) => expr (Entity LogDB) -> (T.Text, T.Text) -> expr (E.Value Bool)
...
toAddrId :: T.Text -> Key AddressStateRef
toAddrId = toId

toBlockId :: T.Text -> Key Block
toBlockId = toId

toId :: ToBackendKey SqlBackend record => T.Text -> Key record
toId v = toSqlKey (fromIntegral $ (read $ T.unpack v :: Integer) )
...
extractPage name ts = Control.Monad.foldM toFold 0 (map selectPage ts)
     where 
...
appendIndex l = map fromParam (Data.Set.toList $ Data.Set.insert (toParam ("index", "")) $ Data.Set.fromList $ map toParam l)
