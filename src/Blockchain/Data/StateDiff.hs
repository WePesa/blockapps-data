...
instance Detailed AccountDiff where
  incrementalToEventual AccountDiff{nonce, balance, code, codeHash, contractRoot, storage} =
    AccountDiff{
      ...
      codeHash = fmap incrementalToEventual codeHash,
...
eventualAccountState 
  ...
  = do
    ...
    return AccountDiff{
      ...
      codeHash = Just $ Value addressStateCodeHash,
      storage
      }
...
incrementalAccountState oldState newState = do
  ...
  return AccountDiff{
    ...
    code = Nothing,
    codeHash = Nothing,--(diff `on` addressStateCodeHash) oldState newState
    storage
    }
